...
def process_polygon_string(polygon_string):
    ...
    for sub_polygon_string in sub_polygon_strings:
        ...
        coordinates = [tuple(map(float, coord.split())) for coord in sub_polygon_string.split(", ")]
...
@time_it
def query_database_and_get_dataframe(connection_str, database_name, table_name, uid, shape_column, spatial_reference, conditions=None, sample=False):
...
df_shapes = query_database_and_get_dataframe(connection_str, database_name, table_name, uid, shape_column, spatial_reference, conditions, sample)
...
df = pd.read_csv(csv_long_lat_file, names = ["unique_identifier", "latitude", "longitude"])
